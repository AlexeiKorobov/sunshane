...
public class WeatherJsonParser {
    ...
    /*
    JSON parsing
    String dayIndex = "1494320400";
    String weatherJsonStr = forecastJsonStr;
    double result = -1;

    JSONObject jObj = new JSONObject(weatherJsonStr);
    JSONArray listDays = jObj.getJSONArray("list");

    for(int i=0; i< listDays.length(); i++){
        JSONObject day = listDays.getJSONObject(i);
        if(day.getString("dt").equals(dayIndex)){
            JSONObject temp = day.getJSONObject("temp");
            result = Double.parseDouble(temp.getString("max"));
